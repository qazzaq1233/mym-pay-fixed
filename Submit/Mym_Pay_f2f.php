...
if($result['code'] && $result['code']=='10000'){
    ...
    $qrcode ="//api.qrserver.com/v1/create-qr-code/?size=150%C3%97150&data=".urlencode($result['qr_code']);
}else{
...
$mp = rand(1,3);
if($mp==1){
$mp3 = "http://tts.youdao.com/fanyivoice?word=尊敬的客户，你好，你本次订单金额为".$srow['price']."元，请按照金额进行付款，否则会导致支付失败&le=zh&keyfrom=speaker-target";
}elseif($mp==2){
    $mp3= "http://tts.youdao.com/fanyivoice?word=尊敬的用户，你本次交易金额为".$srow['price']."元记得不要付错了哟&le=zh&keyfrom=speaker-target";
}else{
    $mp3= "http://tts.youdao.com/fanyivoice?word=请注意查看付款金额避免支付失败&le=zh&keyfrom=speaker-target";
}
?>
...
function Is_Wx_Al(){
    ...
    if( !$.cookie("<?=$trade_no?>") && updateQrImg == 0){
		setTimeout(function(){location.href='Mym_Pay.php?trade_no=<?=$trade_no?>&sitename=<?=$sitename?>';},3000); 
...
#main .right{display:none;}
        ...
        <link rel="stylesheet" type="text/css" href="./Template/default/assets/css/qrcode.css">
</head>
<body>
<div id="main">
	<div class="left">
		<div class="make">
		    <p><img src="./Template/default/assets/img/<?=$type?>-logo.png" alt="" style="height:30px;"></p>
			<p>商品名称：<?=$srow['name']?></p>
			<p class="money" id="price" style="font-weight:bold; color:green"><?=$price?></p>
            <center><p class="qrcode" id="qrcode"><img id="qrcode_load" src="<?=$qrcode?>"></img></p></center>
       <center>
	...
	<div class="right">
		<img src="./Template/default/assets/img/<?=$type?>-sys.png" alt="">
	</div>
</div>
</body>
</html>
<audio autoplay=""><source src="<?php echo $mp3;?>"></audio>
<script src="./Template/default/assets/js/clipboard.min.js"></script>
<script type="text/javascript" src="./Template/default/assets/js/qrcode.min.js"></script>
<script type="text/javascript" src="./Template/default/assets/js/jquery.min.js"></script>
<script type="text/javascript" src="./Template/default/assets/layer/layer.js"></script>
<script>
...
function jscode(){
    var pay_type = '<?=$type?>';//支付方式
    if(pay_type=='alipay'){
        $("#alipayh5url_1").html('<a type="button" href="<?=$alipayh5url_1?>" class="btn btn-lg btn-block btn-success" style="font-size:13px" target="_blank">唤醒支付宝APP支付</a>');
        var url_scheme = '<?=$alipayh5url_1?>';

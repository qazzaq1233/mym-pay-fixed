...
$return_pid = intval($_GET['pid']);
if($name == '测试商品' && $return_pid){
    $test_userrow=$DB->query("SELECT * FROM pay_user WHERE pid='{$return_pid}' limit 1")->fetch();
    if($test_userrow){
...
if($verify_result) {//验证成功
    ...
    if($_GET['trade_status'] == 'TRADE_SUCCESS') {
        if(strpos($name,'购买额度套餐')){
            echo "购买额度套餐成功<br/><a href='/User'>返回用户中心";
        }elseif(strpos($name,'额度充值')){
            echo "开通/充值成功<br/><a href='/User'>返回用户中心";
        }elseif(strpos($name,'通道配额')){
            echo "充值通道配额成功<br/><a href='/User'>返回用户中心";
